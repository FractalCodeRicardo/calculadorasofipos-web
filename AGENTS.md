...
- **No build step.** `index.html` is the only page — plain HTML + external CSS. No framework, bundler, or dependencies.
...
- Styles live in `styles.css` — linked from `<head>` in `index.html`. Avoid element-level `style` attributes.
